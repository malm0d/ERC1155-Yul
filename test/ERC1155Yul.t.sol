...
import {IERC1155} from "test/IERC1155.sol";
 ...
 * - mint failure
 * - burn failure
 * - safeTransferFrom failure
 * - safeBatchTransferFrom failure
 * - batchMint failure
 * - batchBurn failure
 * - balanceOfBatch failure
 * - Fuzz test: mint, batchMint, burn, batchBurn, setApprovalForAll,
 *              safeTransferFrom, safeBatchTransferFrom, balanceOfBatch,
 *
 * - Fuzz failure tests: mint, burn, safeTransferFrom, safeBatchTransferFrom,
 *                          batchMint, batchBurn, balanceOfBatch
 */

//forge test -vvvv --mc ERC1155Test
...
contract ERC1155Test is DSTestPlus, ERC1155TokenReceiver {
    ...
    function testEmitTransferSingle() public {
        ...
        emit IERC1155.TransferSingle(address(this), address(0), address(0xBEEF), 1337, 100);
        token.mint(address(0xBEEF), 1337, 100, "");

        hevm.startPrank(address(0xBEEF));
        address to = address(0xABCD);
        hevm.expectEmit(true, true, true, true);
        emit IERC1155.TransferSingle(address(0xBEEF), address(0xBEEF), to, 1337, 70);
    ...
    function testEmitTransferBatch() public {
        ...
        hevm.expectEmit(true, true, true, true);
        emit IERC1155.TransferBatch(address(this), address(0), address(0xBEEF), ids, amounts);
        token.batchMint(address(0xBEEF), ids, amounts, "");
    }

    function testEmitApprovalForAll() public {
        hevm.expectEmit(true, true, true, true);
        emit IERC1155.ApprovalForAll(address(this), address(0xBEEF), true);
        token.setApprovalForAll(address(0xBEEF), true);
    }
    ...
    //------------------------------------------------------------------------------------------------------------------
    //----------------------------------------Test safeBatchTransferFrom failure----------------------------------------
    //------------------------------------------------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------------------------------------------
    //----------------------------------------Fuzz test: safeBatchTransferFrom----------------------------------------
    //----------------------------------------------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------------------------------------------------
    //----------------------------------------Fuzz test: safeTransferFrom failure----------------------------------------
    //-------------------------------------------------------------------------------------------------------------------
    ...
    //------------------------------------------------------------------------------------------------------------------------
    //----------------------------------------Fuzz test: safeBatchTransferFrom failure----------------------------------------
    //------------------------------------------------------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------------------------------------------
    //----------------------------------------Fuzz test: balanceOfBatch failure----------------------------------------
    //-----------------------------------------------------------------------------------------------------------------
